...
<script>
import { getHomeRotationPictureModify } from '@/api/integrated/news.js'
  ...
  methods: {
    onSubmit() {
      ...
      if (this.fromValiData.type === '1') {
        getHomeRotationPictureModify(this.fromValiData).then((res) => {
        ...
        getHomeHonorWallModify(this.fromValiData).then((res) => {
          this.$layer.close(this.layerid)
          this.$parent.getListData()
          this.$share.message()
          this.btnLoading = false
